...
declare namespace Cypress {
    interface Chainable {
        login(): void;
        logout(): void;
        addExamennaam(examennaam: any): void;
        resetDb(): void;
...
describe('Login Page', () => {
    beforeEach(() => {
        cy.prepTest();
        cy.register('Jelle', 'testtesttest');
        cy.logout();
    });
   ...
   it('should go to the login page and try to login', () => {
       ...
       cy.get('[data-test=username]').type('Jelle');
       cy.get('[data-test=password]').type('testtesttest');
       cy.get('[data-test=loginBtn]').click();

       cy.url().should('include', 'examennaam/list');
       cy.contains('Jelle');
   });
   ...
   it('should show an error when the login is invalid', () => {
       ...
       cy.get('[data-test=username]').type('nietJelle');
       cy.get('[data-test=password]').type('invalid');
       cy.get('[data-test=loginBtn]').click();

       cy.contains('Error 401 while trying to login user nietJelle');
   });
